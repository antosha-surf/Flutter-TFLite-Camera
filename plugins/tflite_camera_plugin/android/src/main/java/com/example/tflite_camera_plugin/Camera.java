...
import com.example.tflite_camera_plugin.features.CameraFeature;
import com.example.tflite_camera_plugin.features.CameraFeatureFactory;
import com.example.tflite_camera_plugin.features.CameraFeatures;
import com.example.tflite_camera_plugin.features.Point;
import com.example.tflite_camera_plugin.features.autofocus.AutoFocusFeature;
import com.example.tflite_camera_plugin.features.autofocus.FocusMode;
import com.example.tflite_camera_plugin.features.exposurelock.ExposureLockFeature;
import com.example.tflite_camera_plugin.features.exposurelock.ExposureMode;
...
import com.example.tflite_camera_plugin.features.flash.FlashMode;
...
import com.example.tflite_camera_plugin.features.sensororientation.DeviceOrientationManager;
import com.example.tflite_camera_plugin.features.zoomlevel.ZoomLevelFeature;
import com.example.tflite_camera_plugin.media.ImageStreamReader;
import com.example.tflite_camera_plugin.media.MediaRecorderBuilder;
import com.example.tflite_camera_plugin.tflite.TFLite;
import com.example.tflite_camera_plugin.types.CameraCaptureProperties;
import com.example.tflite_camera_plugin.types.CaptureTimeoutsWrapper;
...
class Camera
  ...
  /** Encapsulates TFLite logic */
  private final TFLite tflite = new TFLite();
  ...
  public void open(String imageFormatGroup) throws CameraAccessException {
    ...
    // ----------------------------------------------------
    // TFLite: changed image format to YUV420 to be able
    // to feed the resulting image to the TFLite model. todo update this
    pictureImageReader =
  ...
  void initTFLiteSession(String modelPath) throws IOException {
    if (tflite.hasLiveSession()) {
      tflite.disposeSession();
    }
    ...
    if (imageFormat != null && imageFormat != ImageFormat.YUV_420_888) {
      Log.e(
          TAG,
          "Won't start TFLite session unless the camera is running in YUV_420_888 format"
      );
      return;
    }
    ...
    Log.i(TAG, "initTFLiteSession");
  }

  void startTFLiteModelPredictions(EventChannel predictionStreamChannel) throws Exception {
    if (!tflite.hasLiveSession()) {
      throw new Exception("Load a TFLite Model First");
    }

    tflite.startSessionPredictionsStream(predictionStreamChannel, backgroundHandler);

    startCapture(false, false, true);
    Log.i(TAG, "startTFLiteModelPredictions");
  }

  void takePictureAndPredict(Result result) throws Exception {
    if (!tflite.hasLiveSession()) {
      throw new Exception("Load a TFLite Model First");
    }

    if (!tflite.isStreamingPredictions()) {
      throw new Exception("Session must be streaming predictions to call this");
    }

    takePicture(result);
  }

  void disposeTFLiteSession() throws Exception {
    if (!tflite.hasLiveSession()) throw new Exception("No running TFLite session to dispose");
    tflite.disposeSession();
  }
  ...
  private void startCapture(boolean record, boolean stream, boolean tflite) throws CameraAccessException {
    ...
    if (tflite) {
      surfaces.add(this.tflite.getCurrentSessionSurface());
    }
  ...
  public void startVideoRecording(
    ...
    if (imageStreamChannel != null) {
      setImageStreamHandler(imageStreamChannel);
    }
  ...
  public void onImageAvailable(ImageReader reader) {
    ...
    tflite.onImageForInferenceTaken(image, flutterResult);
    image.close();
  }
  ...
  private void setImageStreamHandler(EventChannel imageStreamChannel) {

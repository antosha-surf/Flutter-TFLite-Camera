...
import '../../tflite_camera_platform_interface.dart';
import '../utils/utils.dart';
import 'type_conversion.dart';

const MethodChannel _channel = MethodChannel('TFLiteCamera');
...
class MethodChannelCamera extends CameraPlatform {
  ...
  //
  StreamSubscription<dynamic>? _platformPredictionStreamSubscription;

  // The stream for vending frames to platform interface clients.
  StreamController<CameraImageData>? _frameStreamController;

  StreamController<List<Map<String, dynamic>>>? _predictionStreamController;
  ...
  Future<void> initializeCamera(
    ...
    _channels.putIfAbsent(cameraId, () {
      final MethodChannel channel =
          MethodChannel('TFLiteCamera/camera$cameraId');
      channel.setMethodCallHandler(
  ...
  @override
  Future<PredictionCapture> takePictureAndPredict(int cameraId) async {
    final Map<Object?, Object?>? result =
        await _channel.invokeMethod<Map<Object?, Object?>>(
      'takePictureAndPredictWithTFLite',
      <String, dynamic>{'cameraId': cameraId},
    );
    ...
    if (result == null) {
      throw CameraException(
        'Unexpected result',
        'Call to `takePictureAndPredict` returned unexpected null',
      );
    }

    return PredictionCapture.fromPlatformJson(result.cast<String, dynamic>());
  }
  ...
  Future<void> startVideoCapturing(VideoCaptureOptions options) async {
    ...
    if (options.streamCallback != null) {
      _installFrameStreamController().stream.listen(options.streamCallback);
      _startImageStreamListener();
    }
  }
  ...
  Stream<CameraImageData> onStreamedFrameAvailable(int cameraId,
    ...
    _installFrameStreamController(onListen: _onFrameStreamListen);
    return _frameStreamController!.stream;
  }

  @override
  Stream<List<Map<Object?, Object?>>> onPredictionAvailable() {
    _installPredictionStreamController(onListen: _onPredictionStreamListen);
    return _predictionStreamController!.stream;
  }

  StreamController<CameraImageData> _installFrameStreamController(
  ...
  StreamController<dynamic> _installPredictionStreamController(
      {void Function()? onListen}) {
    _predictionStreamController = StreamController<List<Map<String, dynamic>>>(
      onListen: onListen ?? () {},
      onPause: _onFrameStreamPauseResume,
      onResume: _onFrameStreamPauseResume,
      onCancel: _onPredictionStreamCancel,
    );
    return _predictionStreamController!;
  }

  void _onFrameStreamListen() {
    _startPlatformImageStream();
  }

  Future<void> _startPlatformImageStream() async {
    await _channel.invokeMethod<void>('startImageStream');
    _startImageStreamListener();
  }

  void _startImageStreamListener() {
    const EventChannel cameraEventChannel =
        EventChannel('TFLiteCameraImageStream');
  ...
  void _onPredictionStreamListen() {
    _startPlatformPredictionStream();
  }

  Future<void> _startPlatformPredictionStream() async {
    await _channel.invokeMethod<void>('startTFLiteSessionPredictionStream');
    _startPredictionStreamListener();
  }

  void _startPredictionStreamListener() {
    const EventChannel cameraEventChannel =
        EventChannel('TFLitePredictionsStream');
    _platformPredictionStreamSubscription =
        cameraEventChannel.receiveBroadcastStream().listen((dynamic p) {
      _predictionStreamController!.add((p as List)
  ...
  FutureOr<void> _onPredictionStreamCancel() async {
    await _channel.invokeMethod<void>('stopTFLiteSessionPredictionStream');
    await _platformPredictionStreamSubscription?.cancel();
    _platformPredictionStreamSubscription = null;
    _predictionStreamController = null;
  }
  ...
  Future<void> initTFLiteModel(
    ...
    try {
      ...
      throw TFLiteException(e.code, e.message);
    }
  }

  @override
  Future<void> disposeCurrentTFLiteModel() {
    try {
      return _channel.invokeMethod<void>(
        'disposeTFLiteModel',
      );

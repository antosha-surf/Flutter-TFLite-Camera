...
import 'package:tflite_camera_platform_interface/tflite_camera_platform_interface.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';

import '../tflite_camera_plugin.dart';
...
/// Signature for latest prediction available.
/// This is used by [CameraController.startImageStream].
typedef OnLatestPredictionAvailable = void Function(
...
class CameraValue {
  ...
  const CameraValue({
    ...
    required this.isTFLiteInitialized,
    this.errorDescription,
    this.previewSize,
    required this.isRecordingVideo,
    required this.isTakingPicture,
    required this.isStreamingImages,
    required this.isStreamingPredictions,
  ...
  /// todo docs
  final bool isTFLiteInitialized;
  ...
  /// True when predictions from TFLite are being streamed.
  final bool isStreamingPredictions;
  ...
  CameraValue copyWith({
    ...
    return CameraValue(
      ...
      isTFLiteInitialized: isTFLiteInitialized ?? this.isTFLiteInitialized,
      errorDescription: errorDescription,
      previewSize: previewSize ?? this.previewSize,
      isRecordingVideo: isRecordingVideo ?? this.isRecordingVideo,
      isTakingPicture: isTakingPicture ?? this.isTakingPicture,
      isStreamingImages: isStreamingImages ?? this.isStreamingImages,
      isStreamingPredictions:
          isStreamingPredictions ?? this.isStreamingPredictions,
...
class CameraController extends ValueNotifier<CameraValue> {
  ...
  StreamSubscription<dynamic>? _predictionStreamSubscription;
  ...
  Future<PredictionCapture> takePictureAndPredict() async {
    _throwIfNotInitialized('takePicture');
    if (!value.isTFLiteInitialized) {
      throw CameraException(
        'TFLite session is not initialized',
        'takePictureAndPredict was called before the TFLite '
    ...
    if (value.isTakingPicture) {
      throw CameraException(
        ...
        'takePictureAndPredict was called before the previous capture returned.',
      );
    }
    try {
      value = value.copyWith(isTakingPicture: true);
      final PredictionCapture capture =
          await CameraPlatform.instance.takePictureAndPredict(_cameraId);
  ...
  /// Start streaming predictions from platform camera TFLite binding.
  Future<void> startPredictionStream(OnLatestPredictionAvailable fn) async {
    assert(defaultTargetPlatform == TargetPlatform.android ||
        defaultTargetPlatform == TargetPlatform.iOS);
    _throwIfNotInitialized('startPredictionStream');
    if (value.isRecordingVideo) {
      throw CameraException(
        'A video recording is already started.',
        'startImageStream was called while a video is being recorded.',
    ...
    if (value.isStreamingPredictions) {
      throw CameraException(
        'A camera is streaming predictions.',
        'startPredictionStream was called while a camera was streaming predictions.',
      );
    }

    try {
      _predictionStreamSubscription = CameraPlatform.instance
          .onPredictionAvailable()
          .listen((List<Map<Object?, Object?>> data) {
        fn(data.map((e) => e.cast<String, dynamic>()).toList());
      });
      value = value.copyWith(isStreamingPredictions: true);
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    }
  }
  ...
  /// Stop streaming predictions from platform camera TFLite binding.
  Future<void> stopPredictionStream() async {
    assert(defaultTargetPlatform == TargetPlatform.android ||
        defaultTargetPlatform == TargetPlatform.iOS);
    _throwIfNotInitialized('stopPredictionStream');
    if (!value.isStreamingPredictions) {
      throw CameraException(
        'No camera is streaming predictions',
        'stopPredictionStream was called when no camera is streaming predictions.',
      );
    }

    try {
      await _predictionStreamSubscription?.cancel();
      _predictionStreamSubscription = null;
      value = value.copyWith(isStreamingPredictions: false);
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    }
  }
  ...
  Future<void> initializeTFLiteModel(
    String modelAssetPath,
  ) async {
    try {
      await CameraPlatform.instance.initTFLiteModel(
        modelAssetPath,
      );
      value = value.copyWith(isTFLiteInitialized: true);
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
  ...
  Future<void> disposeCurrentTFLiteModel() async {
    try {
      if (value.isStreamingPredictions) {
        await stopPredictionStream();
      }
      await CameraPlatform.instance.disposeCurrentTFLiteModel();
      value = value.copyWith(isTFLiteInitialized: false);
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    } on TFLiteException catch (e) {
      throw CameraException(e.code, e.description);
